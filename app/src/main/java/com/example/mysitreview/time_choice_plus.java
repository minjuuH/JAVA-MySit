...
public class time_choice_plus extends AppCompatActivity {
    ...
    public void btnEventListener(){
        ...
        mDatabase.child("장소 글작성 정보").child(title).addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                for(DataSnapshot snapshot : dataSnapshot.getChildren()){
                    WritePostClass wp = dataSnapshot.getValue(WritePostClass.class);
                    content = wp.getIntroduce();
                    Map<String, Object> taskMap = new HashMap<String, Object>();
